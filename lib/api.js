...
function generateListingUrl(citySlug, listingSlug, type, isSold = false) {
  ...
  switch (type) {
    case "resale":
      return `/resale/ontario/${safeCity}/listings${isSold ? "/sold" : ""}/${safeListing}`;
...
async function fetchListingsForCity(city, type = "pre-construction", includeSold = false) {
  try {
    if (type === "resale") {
      // For resale, we'll get the listings from the AMPRE API
      const filterQuery = `$filter=contains(City,'${city.name}') and TransactionType eq '${includeSold ? "Sold" : "For Sale"}'`;
      ...
      if (!data?.value || !Array.isArray(data.value)) {
        console.warn(`No ${includeSold ? "sold" : ""} ${type} listings found for ${city.name}`);
        return [];
      }
      ...
      return data.value.map((listing) => {
        ...
        return {
          ...
          url: generateListingUrl(cleanSlug(city.name), `${address}-${listing.ListingId}`, type, includeSold),
      ...
      const endpoint = `${API_BASE_URL}/pre-constructions-city/${cleanSlug(city.slug)}/?perpage=100&page=1`;
...
export async function getAdditionalRoutes() {
  const resaleRoutes = [
    ...
    ...preconCityList.map((city) => ({
      path: `resale/ontario/${cleanSlug(city.city_name)}`,
      priority: 0.8,
      changefreq: "daily",
      subpages: [
        "detached-homes-for-sale",
