    ...
    if (type === 'resale') {
      ...
      return data.value.map(listing => {
        ...
        return {
          ...
          url: generateListingUrl(cleanSlug(city.name), `${address}-${listing.ListingId}`, type),
...
export async function getAllListings() {
  try {
    ...
    const ontarioListings = await fetchListingsForCity({ name: 'Ontario', slug: 'ontario' }, 'resale');
